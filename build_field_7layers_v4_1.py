# build_field_7layers_v4_1.py — СЕМИСЛОЙНОЕ ПОЛЕ v4.1: ВММП-демократия + оптимизация памяти
...
print("=" * 60)
print("🏗️ СЕМИСЛОЙНОЕ ПОЛЕ v4.1: ВММП-демократия + оптимизация")
print("=" * 60)

INPUT_FILE = 'dialogue_texts.json'
OUTPUT_JSON = 'src/rizoma/data/personalities/p016_7layers_v4_1.json'
OUTPUT_DB = 'src/rizoma/data/personalities/text_store_7layers_v4_1.db'
...
lp = LivingPersonality(id="p016_7layers_v4_1", name="p016 ВММП-демократия", db_path=OUTPUT_DB)

# ═══════════════════════════════════════════════════════
# ВММП-ДЕМОКРАТИЯ: мутации только на верхних слоях
# ═══════════════════════════════════════════════════════

MUTATION_RATES = {
    1: 0.0,   # Буквы — НИКОГДА (алфавит фиксирован)
    2: 0.0,   # Слоги — НИКОГДА (только из существующих букв)
    3: 0.0,   # Слова — НИКОГДА (объясняй через фразы!)
    4: 0.0,   # Частые слова — НИКОГДА
    5: 0.05,  # Фразы — умеренно
    6: 0.1,   # Сильные фразы — можно
    7: 0.5,   # Тексты — активно
}

RELAXATION_PERIODS = {1: 0, 2: 0, 3: 0, 4: 0, 5: 10, 6: 5, 7: 2}

print(f"\n📊 ВММП-демократия активирована:")
print(f"   Слои 1-4: мутации ЗАПРЕЩЕНЫ (объясняй через фразы!)")
print(f"   Слои 5-7: мутации РАЗРЕШЕНЫ (новые смыслы из старых слов)")
print(f"   Бритва Оккама: не плоди новые термины без необходимости")

# ═══════════════════════════════════════════════════════
# СТАТИСТИКА (оптимизированная — без лишних индексов)
# ═══════════════════════════════════════════════════════

print(f"\n📊 Сбор статистики (оптимизированный)...")
...
syllable_to_letters = defaultdict(set)  # слог → буквы (для наследования)
word_to_syllables = defaultdict(set)    # слово → слоги (для наследования)
phrase_to_words = defaultdict(set)      # фраза → слова (для наследования)
...
for msg_idx, item in enumerate(all_texts):
    ...
    for j in range(len(words) - 1):
        ...
        phrase_to_words[f"{words[j]} {words[j+1]}"].add(words[j])
        phrase_to_words[f"{words[j]} {words[j+1]}"].add(words[j+1])
...
print(f"   Память после статистики: {get_mb():.0f} MB")
...
print(f"\n🔨 Создание мод с ВММП-демократией...")
print(f"   Память перед модами: {get_mb():.0f} MB")
start = time.time()
...
for letter, freq in letter_freq.items():
    mode = SpectralMode(
        tau=ord(letter) % 30 + 1, amplitude=0.05 + 0.1 * (freq / max_letter), scale=0.5,
        trace_id=f"L1_{letter}", creator="layer1", content=letter,
        emotion=WaveformEmotion.from_string('neutral', 0.1),
    ...
    total_modes += 1
print(f"      {total_modes} мод, RAM: {get_mb():.0f} MB")
...
for syl, freq in syllable_freq.items():
    if len(syl) >= 1:
        ...
        mode_index[mode.trace_id] = mode
        total_modes += 1
print(f"      +{len(syllable_freq)} = {total_modes} мод, RAM: {get_mb():.0f} MB")

# Очищаем syllable_to_letters — больше не нужен
del syllable_to_letters
gc.collect()
print(f"   После очистки: {get_mb():.0f} MB")
...
for word, freq in word_freq.items():
    ...
    mode._mutation_allowed = False  # ЗАПРЕЩЕНО!
    lp.add_mode(mode)
    mode_index[mode.trace_id] = mode
    total_modes += 1
print(f"      +{len(word_freq)} = {total_modes} мод, RAM: {get_mb():.0f} MB")

# Очищаем word_to_syllables
...
for (w1, w2), count in pairs.most_common(100000):
    ...
    mode._mutation_allowed = True  # РАЗРЕШЕНО
    lp.add_mode(mode)
    mode_index[mode.trace_id] = mode
    total_modes += 1
print(f"      +{len(phrase_seen)} = {total_modes} мод, RAM: {get_mb():.0f} MB")

# СЛОЙ 7: ТЕКСТЫ (без сохранения в SQLite)
...
for msg_idx, item in enumerate(all_texts):
    ...
    mode.text_id = ''  # НЕ сохраняем в SQLite (экономим память)
    mode._mutation_allowed = True
    lp.add_mode(mode)
    mode_index[mode.trace_id] = mode
    total_modes += 1
    layer7_count += 1
print(f"      +{layer7_count} = {total_modes} мод, RAM: {get_mb():.0f} MB")

# Очищаем всё лишнее перед TEES
del phrase_to_words, phrase_seen, pairs
gc.collect()
print(f"   После финальной очистки: {get_mb():.0f} MB")
...
# Пересобираем пары из word_freq (pairs уже удалён, но word_freq остался)
# Используем сохранённые фразы из mode_index для построения каналов
channels = defaultdict(list)
channel_count = 0
for tid, mode in mode_index.items():
    if tid.startswith('L5_'):
        parts = tid[3:].rsplit('_', 1)
        if len(parts) == 2:
            w1, w2 = parts
            if f"L3_{w1}" in mode_index and f"L3_{w2}" in mode_index:
                # Приблизительная сила канала из энергии фразы
                strength = mode.energy
                channel = type('Channel', (), {
                    'from_word': w1, 'to_word': w2, 'strength': strength,
                    'mutual_info': 0, 'count': 1
                })()
                channels[w1].append(channel)
                channel_count += 1

print(f"   Каналов из фраз: {channel_count}")
...
for cycle in range(CYCLES):
    ...
    for word in sources:
        ...
        if energy_diff > 0:
            ...
            for mid in [mode_from, mode_to]:
                if hasattr(mid, 'parent_ids'):
                    for pid in mid.parent_ids:
