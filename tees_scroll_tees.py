# tees_scroll.py
# 📜 Свиток (мнемоника) и Портал (адрес)
...
BIP39_WORDS_FILE = Path(__file__).parent / 'bip39_words.json'
...
BIP39_WORDS_BUILTIN = [
    ...
    "equip", "era", "erase", "erode", "erosion", "error", "erupt", "escape", "essay",
    ...
    "fluid", "flush", "fly", "foam", "focus", "fog", "foil", "fold", "follow",
    ...
    "never", "news", "next", "nice", "night", "noble", "noise", "nominee", "noodle",
    "normal", "north", "nose", "notable", "note", "nothing", "notice", "novel", "now",
    "nuclear", "number", "nurse", "nut", "oak", "obey", "object", "oblige", "obscure",
    ...
    "practice", "praise", "predict", "prefer", "prepare", "present", "pretty",
    ...
    "scout", "scrap", "screen", "script", "scrub", "sea", "search", "season", "seat",
    ...
    "sheriff", "shield", "shift", "shine", "ship", "shiver", "shock", "shoe", "shoot",
...
def load_bip39_words():
    if BIP39_WORDS_FILE.exists():
        with open(BIP39_WORDS_FILE, 'r', encoding='utf-8') as f:
            w = json.load(f)
            if len(w) == 2048:
                return w
    BIP39_WORDS_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(BIP39_WORDS_FILE, 'w', encoding='utf-8') as f:
        json.dump(BIP39_WORDS_BUILTIN, f)
    return BIP39_WORDS_BUILTIN
...
def create_scroll(strength=128):
    if strength not in [128, 256]:
        strength = 128
    system_noise = hashlib.sha256(str(time.time_ns()).encode() + os.urandom(32)).digest()
    entropy = secrets.token_bytes(strength // 8)
    entropy = hashlib.sha256(entropy + system_noise).digest()[:strength // 8]
    h = hashlib.sha256(entropy).digest()
    cs_bits = strength // 32
    checksum = h[0] >> (8 - cs_bits)
    entropy_bits = int.from_bytes(entropy, 'big')
    combined = (entropy_bits << cs_bits) | checksum
    word_count = (strength + cs_bits) // 11
    return " ".join(
        BIP39_WORDS[(combined >> (i * 11)) & 0x7FF]
        for i in range(word_count - 1, -1, -1)
    )


def scroll_to_seed(scroll, passphrase=""):
    return hashlib.pbkdf2_hmac(
        'sha512', scroll.encode(),
...
def validate_scroll(scroll):
    ...
    ent = total - cs
    combined = sum(
        idx << (11 * (len(indices) - 1 - i))
        for i, idx in enumerate(indices)
    )
    ent_bytes = (combined >> cs).to_bytes(ent // 8, 'big')
    return (combined & ((1 << cs) - 1)) == (
        hashlib.sha256(ent_bytes).digest()[0] >> (8 - cs)
    )
...
def portal_from_hash(hash_bytes):
    ...
    return '1' + ''.join(reversed(res))


def scroll_to_portal(scroll, depth=3):
    seed = scroll_to_seed(scroll)
    mk = seed_to_master_key(seed)
    init_seed = hashlib.sha256(mk).digest()
    fh = tees_recursive_vortex(scroll.encode(), init_seed, depth)
    th = tees_triad_collapse(fh)
    return portal_from_hash(th)


def mine_child_portals(scroll, count=5):
    seed = scroll_to_seed(scroll)
    mk = seed_to_master_key(seed)
    portals = []
    for i in range(count):
        ck = hmac.new(
            b'Bitcoin seed',
            b'\x00' + mk + struct.pack('>I', i + 0x80000000),
            hashlib.sha512
        ).digest()[:32]
        init_seed = hashlib.sha256(ck).digest()
        fh = tees_recursive_vortex(f"{scroll}:{i}".encode(), init_seed, 3)
        th = tees_triad_collapse(fh)
        portals.append({
            'index': i,
            'portal': portal_from_hash(th),
            'path': f"m/44'/0'/0'/0/{i}"
        })
    return portals