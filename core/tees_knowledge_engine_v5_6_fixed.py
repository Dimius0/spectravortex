...
try:
    ...
    print("⚠️ pymorphy3 не найден — только стоп-слова")
...
class TeesParser:
    ...
    STOP_WORDS_RU = frozenset({
        ...
        # Модальные и связки
        'может', 'могут', 'можно', 'нужно', 'надо', 'должен', 'должна',
        'есть', 'быть', 'был', 'была', 'было', 'были', 'будет', 'будут',
        'бывает', 'бывают', 'стал', 'стала', 'стало', 'стали',
        # Наречия и вводные
        'например', 'однако', 'более', 'менее', 'также', 'только', 'даже',
        'уже', 'ещё', 'еще', 'снова', 'опять', 'теперь', 'тогда', 'потом',
        'почти', 'весьма', 'совсем', 'вполне', 'именно', 'просто', 'ровно',
        'примерно', 'очень', 'много', 'мало', 'хорошо', 'плохо',
        # Местоимения
        'этот', 'эта', 'это', 'эти', 'такой', 'такая', 'такое', 'такие',
        'который', 'которая', 'которое', 'которые', 'которого', 'которой',
        'которых', 'которым', 'которыми', 'котором',
        'весь', 'вся', 'всё', 'все', 'всего', 'всех', 'всем', 'всеми',
        'тот', 'та', 'те', 'того', 'той', 'тех', 'тем', 'теми',
        'там', 'тут', 'здесь', 'сейчас',
        # Вопросительные
        'где', 'куда', 'откуда', 'почему', 'зачем', 'сколько', 'насколько',
        'когда', 'пока', 'если', 'чтобы', 'хотя', 'ведь', 'раз',
        # Временные
        'всегда', 'никогда', 'часто', 'редко', 'обычно', 'иногда', 'вдруг',
        'наконец', 'сначала', 'затем', 'далее', 'итак',
        # Прочие
        'тоже', 'либо', 'нибудь', 'каждый', 'любой', 'некоторый', 'другой',
        'другие', 'другая', 'другое', 'других', 'другим', 'другими',
    })
    ...
    STOP_WORDS_EN = frozenset({
        ...
        'also', 'too', 'either', 'neither', 'both', 'all', 'none',
        'here', 'there', 'now', 'then', 'always', 'never', 'often',
        'sometimes', 'usually', 'already', 'yet', 'still', 'again',
        'just', 'only', 'even', 'almost', 'quite', 'rather', 'very',
        'well', 'badly', 'good', 'bad', 'better', 'worse',
        # Местоимения
        'this', 'these', 'those', 'such', 'same', 'other', 'another',
        'some', 'any', 'every', 'each', 'no', 'none', 'all', 'both',
        'one', 'ones', 'thing', 'things', 'way', 'ways',
        # Предлоги
        'into', 'onto', 'upon', 'within', 'without', 'through',
        'during', 'before', 'after', 'between', 'among', 'under',
        'over', 'above', 'below', 'across', 'along', 'around',
    })

    FORBIDDEN_TEES = frozenset({
        # Русские предлоги, союзы, частицы
        'в', 'во', 'на', 'с', 'со', 'к', 'ко', 'по', 'из', 'от',
        'для', 'без', 'над', 'под', 'об', 'обо', 'при', 'за', 'до',
        'через', 'между', 'перед', 'около', 'вокруг', 'после',
        'про', 'ради', 'сквозь', 'вдоль', 'и', 'а', 'но', 'или',
        'либо', 'да', 'же', 'бы', 'ли', 'не', 'ни', 'то', 'что',
        'как', 'так', 'это', 'если', 'чтобы', 'хотя', 'ведь',
        # Английские
    ...
    @classmethod
    def is_content_word(cls, word: str) -> bool:
        """
        Слово — значимое (существительное, глагол, прилагательное)?
        
        Использует pymorphy3 если доступен.
        """
        if not PYMORPHY_AVAILABLE:
            # Fallback: только длина и не стоп-слово
            return len(word) > 2 and word.lower() not in cls.STOP_WORDS_RU
        
        parsed = MORPH.parse(word)[0]
        pos = parsed.tag.POS
        return pos in {'NOUN', 'VERB', 'ADJF', 'ADJS', 'INFN', 'PRTF', 'PRTS', 'GRND'}
    ...
    def parse(cls, text: str) -> dict:
        ...
        for sent in sentences:
            ...
            for i in range(1, len(words) - 1):
                ...
                # НОВОЕ: tees не может быть предлогом/союзом/частицей
                if word in cls.FORBIDDEN_TEES:
                    continue
                
                # НОВОЕ: tees должен быть значимым словом (для русского)
                if is_russian and not cls.is_content_word(word):
                    continue
                ...
                # НОВОЕ: source и receiver тоже должны быть значимыми (для русского)
                if is_russian:
                    if not cls.is_content_word(words[source_idx]):
                        continue
                    if not cls.is_content_word(words[receiver_idx]):
                        continue
...
class VMMPChecker:
    ...
    def _compute_vortex_raw(self, word_hash: str) -> np.ndarray:
        ...
        seed = int(word_hash[:8], 16) ^ self.seed  # 32 бита от TEES-вихря
        return seed_to_vortex(seed, self.vortex_config)

    def get_vortex(self, word: str) -> np.ndarray:
        if not word or not word.strip():
            return np.zeros((self.vortex_config.grid_size,) * 2, dtype=self.vortex_config.dtype)
        from core.tees_core_tees import tees_recursive_vortex
        vortex_bytes = tees_recursive_vortex(
            word.encode('utf-8'),
            b'tees_seed',
            3
        )
        word_hash = vortex_bytes.hex()
