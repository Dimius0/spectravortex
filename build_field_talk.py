# build_field_talk.py — РАЗГОВОРНОЕ ПОЛЕ на 7 слоях
...
print("=" * 60)
print("💬 РАЗГОВОРНОЕ ПОЛЕ на 7 слоях")
print("=" * 60)

INPUT_FILE = 'talk_texts.json'
OUTPUT_JSON = 'src/rizoma/data/personalities/p016_talk.json'
OUTPUT_DB = 'src/rizoma/data/personalities/text_store_talk.db'
...
lp = LivingPersonality(id="p016_talk", name="p016 Разговорное поле", db_path=OUTPUT_DB)
...
for letter, freq in letter_freq.items():
    ...
    mode = SpectralMode(tau=ord(letter) % 30 + 1, amplitude=energy, scale=0.5,
        trace_id=f"L1_{letter}", creator="talk_L1", content=letter,
...
for syl, freq in syllable_freq.items():
    ...
    mode = SpectralMode(tau=(len(syl) * 3 + sum(ord(c) for c in syl[:3]) % 20) % 30 + 2,
        amplitude=energy, scale=1.5, trace_id=f"L2_{syl}", creator="talk_L2", content=syl,
        emotion=WaveformEmotion.from_string('neutral', energy),
...
for word, freq in word_freq.items():
    ...
    mode = SpectralMode(tau=(word_hash[0] % 50) + 5.0, amplitude=energy, scale=scale,
        trace_id=f"L3_{word}", creator="talk_L3", content=word,
        emotion=WaveformEmotion.from_string('neutral', energy),
...
phrase_seen = set()
for (w1, w2), count in pairs.most_common(50000):
    phrase = f"{w1} {w2}"
    ...
    if mi < 5: continue  # ниже порог для разговоров
    ...
    mode = SpectralMode(tau=(len(w1) + len(w2)) % 40 + 8,
        ...
        trace_id=f"L5_{w1}_{w2}", creator="talk_L5", content=phrase,
        emotion=WaveformEmotion.from_string('neutral', energy),
...
for msg_idx, item in enumerate(all_texts):
    ...
    mode = SpectralMode(tau=(len(text) / 20) % 40 + 32, amplitude=energy,
        ...
        trace_id=f"L7_{text_hash}", creator="talk_L7", content=text[:300],
...
print(f"\n{'='*60}")
print(f"🌀 КАНАЛЬНЫЙ TEES: разговорное поле")
print(f"{'='*60}")
...
# Текстовые каналы через фразы
text_channels = defaultdict(list)
print(f"   Построение текстовых каналов...")
phrase_count = 0
for tid, mode in mode_index.items():
    if not tid.startswith('L5_'): continue
    phrase = mode.content
    ...
    if len(text_ids) >= 2:
        strength = mode.energy / max(len(text_ids), 1)
        for i in range(len(text_ids)):
            for j in range(i+1, len(text_ids)):
                text_channels[text_ids[i]].append((text_ids[j], strength))
                text_channels[text_ids[j]].append((text_ids[i], strength))
    phrase_count += 1
    if phrase_count % 5000 == 0:
        print(f"   Фраз: {phrase_count}/{len(phrase_seen)}, каналов: {sum(len(v) for v in text_channels.values())}")
...
for cycle in range(CYCLES):
    ...
    if all_text_sources:
        sources = random.sample(all_text_sources, min(500, len(all_text_sources)))
        for tid in sources:
...
for i, m in enumerate(top_texts):
    content = m.content[:120] if m.content else '(пусто)'
...
# Добавим агрессивную очистку ПЕРЕД сохранением, не трогая контент

# ... (весь код до сохранения) ...
...
# 1. Удаляем все индексы и временные структуры
del mode_index, word_channels, text_channels
del all_word_sources, all_text_sources
del phrase_to_texts, phrase_seen

# 2. Очищаем моды от временных атрибутов (контент НЕ трогаем!)
