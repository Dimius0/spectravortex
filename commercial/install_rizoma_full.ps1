# install_rizoma_full.ps1
# Полная установка Ризомы (8 модулей)

Write-Host "🚀 ПОЛНАЯ УСТАНОВКА РИЗОМЫ (8 МОДУЛЕЙ)" -ForegroundColor Magenta
...
# Compile
Write-Host "🔨 Компиляция 8 модулей..." -ForegroundColor Yellow
...
class RizomaCore:
    def __init__(self, freq=60.0):
        print(f"⚙️ Инициализация ядра (8 модулей)")
    ...
    def start(self):
        self.tick.Start()
        self.running = True
        print("✅ Ризома запущена (8 модулей)")
...
print("⚡ Тест 8 модулей (5 секунд)...")
for i in range(5):
    time.sleep(1)
    core.feed_pulse(9.8 + i*0.1)
    status = core.get_status()
    print(f"   Сек {i+1}: такт={status['tick']}, вектор={status['vector']:.3f}, синх={status['sync']}")

core.stop()
print(f"✅ Итог: такт={core.get_tick()}")
"@ | Out-File -FilePath "$corePath\test_full.py" -Encoding ASCII

Write-Host @"
╔══════════════════════════════════════════════════════════════╗
║  ✅ ПОЛНАЯ УСТАНОВКА ЗАВЕРШЕНА (8 МОДУЛЕЙ)                  ║
║  📍 Core: $corePath\rizoma_core.py                          ║
║  📍 Test: $corePath\test_full.py                            ║
║  📍 DLL:  $binPath\RizomaModules.dll                        ║
╠══════════════════════════════════════════════════════════════╣
║  ▶️ Тест: python "$corePath\test_full.py"                   ║
╚══════════════════════════════════════════════════════════════╝
