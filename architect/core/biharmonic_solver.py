"""
Решатель бигармонического уравнения для топологического синтеза.
Версия 3.0.0 с LIL-матрицами и полной функциональностью.
"""
...
class BiharmonicSolver:
    ...
    def __init__(self, grid_shape, spacing=(1.0, 1.0, 1.0)):
        ...
        print(f"[BIHARMONIC] Инициализирован решатель для сетки {grid_shape} ({self.n_points} точек)")
    ...
    def solve(self, vortices, boundary_conditions=None):
        ...
        # Статистика решения
        print(f"[BIHARMONIC] Решение завершено.")
        print(f"[BIHARMONIC]   min(φ) = {phi_field.min():.3f}")
        print(f"[BIHARMONIC]   max(φ) = {phi_field.max():.3f}")
        print(f"[BIHARMONIC]   mean(φ) = {phi_field.mean():.3f}")
    ...
    def compute_gradient_field(self, phi_field):
        """Вычисляет векторное поле градиента ∇φ."""
        grad_x = np.gradient(phi_field, axis=2)
        grad_y = np.gradient(phi_field, axis=1)
        grad_z = np.gradient(phi_field, axis=0)
...
def test_solver():
    ...
    print("=== ТЕСТ РЕШАТЕЛЯ БИГАРМОНИЧЕСКОГО УРАВНЕНИЯ v3.0 ===")
    ...
    print(f"[BIHARMONIC] Вычисленная энергия поля: {energy:.6f}")
