...
class TEES_Layer:
    ...
    def interpolate_batch(self, points: np.ndarray) -> np.ndarray:
        ...
        # Кэшируем результат
        if len(self.interpolation_cache.cache) < self.interpolation_cache.max_size:
...
class TEES_TripleHub:
    ...
    def __init__(self, field_size: int = 32, use_gpu: bool = False, 
        ...
        # Кэш маршрутов (ТОЛЬКО на уровне хаба)
    ...
    def _calculate_optimal_batch_size(self) -> int:
        ...
        max_points = int((available_gb * 0.5) * 1e9 / 100)
    ...
    def route_cross_batch_optimized(self, pos_top: Dict, pos_bot: Dict, 
        ...
        for start_idx in range(0, M, self.optimal_batch_size):
            ...
            # Очищаем кэш после каждых 5 подбатчей
    ...
    def _route_cross_single_batch(self, pos_top: Dict, pos_bot: Dict, 
        ...
        for i in range(M):
            if total_quality[i] > 0.7:
                # Усиливаем поле вдоль успешного пути
                path = np.vstack([
                    top_starts[i].reshape(1, 3),
                    hub_entries[i].reshape(1, 3),
                    hub_exits[i].reshape(1, 3),
                    bot_ends[i].reshape(1, 3)
                ])
                self._reinforce_path(path, total_quality[i])
                paths_count += 1
        
        return total_quality, paths_count

    def _reinforce_path(self, path: np.ndarray, quality: float):
        """Усиление поля вдоль успешного маршрута."""
        quality = min(quality, 1.0)
        # Усиливаем все три слоя
        for layer in [self.top, self.hub, self.bot]:
            for point in path:
                x, y, z = int(np.floor(point[0])), int(np.floor(point[1])), int(np.floor(point[2]))
                if (0 <= x < layer.field_size - 1 and 
                    0 <= y < layer.field_size - 1 and 
                    0 <= z < layer.field_size - 1):
                    boost = quality * 0.05 * np.exp(1j * np.angle(layer.field[x, y, z]))
                    layer.field[x, y, z] += boost
    ...
    def run_parallel_test_optimized(self, n_qubits: int = 500, epochs: int = 3,
        ...
        for epoch in range(epochs):
            ...
            try:
                ...
                logger.error(f"Ошибка в эпохе {epoch+1}: {e}")
                raise
    ...
    def _process_layers_parallel(self, edges_top, edges_bot, pos_top, pos_bot):
        ...
        with ProcessPoolExecutor(max_workers=2) as executor:
            future_top = executor.submit(
                _process_layer_chunk, 'top', edges_top, pos_top, pos_top
            )
            future_bot = executor.submit(
                _process_layer_chunk, 'bot', edges_bot, pos_bot, pos_bot
            )
            
            q_top = future_top.result(timeout=60)
            q_bot = future_bot.result(timeout=60)
    ...
    def _print_epoch_stats(self, epoch: int, q_top: np.ndarray, q_bot: np.ndarray,
        ...
        total_quality = (top_avg + bot_avg + cross_avg) / 3
    ...
    def _print_final_stats(self, total_time: float, initial_memory: float, 
        ...
        total_cache_hits = (self.top.metrics.cache_hits + 
                           self.hub.metrics.cache_hits + 
                           self.bot.metrics.cache_hits)
        ...
        logger.info(f"📊 Кэш хитов: {total_cache_hits}")
        logger.info(f"{'='*60}")
...
def _process_layer_chunk(layer_name: str, edges: List, 
    ...
    """Функция для параллельной обработки слоя."""
