# build_field_v5_1.py — ПОЛНОЕ ПОЛЕ v5.1: TEES-каналы через фразы (потоково)
...
print("=" * 60)
print("🏗️ ПОЛНОЕ ПОЛЕ v5.1: TEES-каналы через фразы (потоково)")
print("=" * 60)

INPUT_FILE = 'dialogue_texts.json'
OUTPUT_JSON = 'src/rizoma/data/personalities/p016_v5_1.json'
OUTPUT_DB = 'src/rizoma/data/personalities/text_store_v5_1.db'
...
lp = LivingPersonality(id="p016_v5_1", name="p016 v5.1 потоковый TEES", db_path=OUTPUT_DB)

total_modes = 0
mode_index = {}
start = time.time()

# СЛОЙ 1-6 (без изменений — быстро)
...
for syl, freq in syllable_freq.items():
    ...
    mode = SpectralMode(tau=(len(syl) * 3 + sum(ord(c) for c in syl[:3]) % 20) % 30 + 2,
        amplitude=energy, scale=1.5, trace_id=f"L2_{syl}", creator="layer2", content=syl,
        emotion=WaveformEmotion.from_string('neutral', energy),
...
del syllable_to_letters, letter_to_syllables; gc.collect()
...
del word_to_syllables; gc.collect()
...
for (w1, w2), count in pairs.most_common(100000):
    ...
    mode = SpectralMode(tau=(word_freq.get(w1, 0) + word_freq.get(w2, 0)) % 40 + 8,
...
# Очистка тяжёлых индексов
del text_to_phrases, word_to_phrases, phrase_to_words, pairs
gc.collect()
...
for layer_id in range(1, 8):
    print(f"   Слой {layer_id}: {layer_counts.get(layer_id, 0):8d} мод")

# ═══════════════════════════════════════════════════════
# ШАГ 4: КАНАЛЬНЫЙ TEES (слова + тексты ПОТОКОВО)
# ═══════════════════════════════════════════════════════

print(f"\n{'='*60}")
print(f"🌀 КАНАЛЬНЫЙ TEES: слова + тексты через фразы (ПОТОКОВО)")
print(f"{'='*60}")

# Каналы между словами
word_channels = defaultdict(list)
for (w1, w2), count in [(('',''),0)]: pass  # уже удалены, используем фразы

# Вместо этого: строим каналы между словами через mode_index фраз
...
# Каналы между текстами — ПОТОКОВО через phrase_to_texts!
text_channels = defaultdict(list)
print(f"   Построение текстовых каналов через фразы (потоково)...")
...
for tid, mode in mode_index.items():
    ...
    # Все тексты, содержащие эту фразу
    ...
    if len(text_ids) >= 2:
        ...
        # Связываем все тексты с этой фразой попарно
    ...
    phrase_count += 1
    if phrase_count % 10000 == 0:
        print(f"   Фраз обработано: {phrase_count}/{len(phrase_seen)}, каналов: {sum(len(v) for v in text_channels.values())}")
