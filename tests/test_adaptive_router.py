...
class TestAStarRouter:
    ...
    def test_timeout_protection(self):
        """Test that A* times out on impossible maze - SLOW TEST"""
        router = AStarRouter(grid_size=0.1)  # Normal grid for speed

        # Create simpler maze that will timeout
        obstacles = []
        for i in range(10):  # Reduced from 20
            obstacles.append((i * 0.2, 0.0, i * 0.2 + 0.1, 1.0))
...
class TestWavefrontRouter:
    ...
    def test_wavefront_maze(self):
        ...
        obstacles = [
            (0.3, 0.3, 0.5, 0.7),  # Vertical block
        ]
        self.router.set_obstacles(obstacles)
...
class TestAdaptiveRouterIntegration:
    ...
    def test_algorithm_fallback(self):
        ...
        # Create simple maze
        obstacles = []
        for i in range(3):  # Reduced complexity
            obstacles.append((0.3, i * 0.3, 0.7, i * 0.3 + 0.1))
    ...
    def test_performance_basic(self):
        """Basic performance test - SLOW TEST"""
        router = create_adaptive_router(grid_size=0.2)  # Coarser grid for speed

        start = (0.0, 0.0)
        end = (3.0, 3.0)  # Shorter path
...
class TestEdgeCases:
    ...
    def test_very_close_points(self):
        ...
        router = create_adaptive_router(grid_size=0.05)
    ...
    def test_large_coordinates(self):
        ...
        router = create_adaptive_router(grid_size=1.0)

        start = (0.0, 0.0)
        end = (10.0, 10.0)  # Reduced distance
    ...
    def test_many_obstacles(self):
        """Stress test with many obstacles - SLOW TEST"""
        router = create_adaptive_router(grid_size=0.2)

        # Create grid of obstacles
        obstacles = []
        for i in range(4):  # Reduced from 5
            for j in range(4):
...
def run_all_tests():
    ...
    print("=" * 60)
    print("Running ALL Adaptive Router Tests")
    print("=" * 60)
    ...
    exit_code = pytest.main([
        ...
        "--tb=short",
        "--disable-warnings",
        "--timeout=30",  # 30 second timeout for entire suite
    ])
...
def run_fast_tests_only():
    """Run only fast tests"""
    print("=" * 60)
    print("Running FAST Adaptive Router Tests Only")
    print("=" * 60)
