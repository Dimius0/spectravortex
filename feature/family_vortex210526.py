#!/usr/bin/env python3
"""
family_vortex.py v3.2.2 — фрактально-эмерджентная модель.
Бигармоническое поле. Обменные сущности в зазорах. Фрактальное время.
Динамическая регенерация/деградация.
+ Исправлено: деление на ноль, нулевая энергия поля, рождение по датам.
"""
...
class BiharmonicField:
    ...
    def evolve(self, positions_list, charges_list, tensions, dt=0.1):
        ...
        denominator = 1.0 + dt * (self.k_biharmonic + self.gamma)
...
class ResonanceDetector:
    ...
    def find_resonances(self, clusters, field):
        entities = []
        born = [c for c in clusters if c.born and not c.dissolved]
        if len(born) < 2:
            return entities
        
        for i, c1 in enumerate(clusters):
            if not c1.born or c1.dissolved:
                continue
            for j, c2 in enumerate(clusters):
                if i >= j or not c2.born or c2.dissolved:
                    continue
...
class FamilyEvolution:
    """Эволюция семейного кластера v3.2.2."""
    ...
    def evolve_step(self, noise_level=0.005):
        ...
        # Пересчитываем поле при рождении
        ...
        for idx, cluster in enumerate(self.clusters):
            ...
            for i in range(cluster.n_vortices):
                original = cluster.original_tau_charges[i]
            ...
            # Фазовый переход
            if cluster.strain > 2.0 and cluster.n_vortices > 1:
                charges = cluster.tau_charges
                non_zero = [i for i, t in enumerate(charges) if abs(t) > 1e-6]
                if len(non_zero) >= 2:
                    strongest = max(non_zero, key=lambda i: abs(charges[i]))
                    weakest = min(non_zero, key=lambda i: abs(charges[i]))
                    transfer = charges[strongest] * 0.1
                    charges[strongest] -= transfer
                    charges[weakest] += transfer
                cluster.strain *= 0.5
            
            if cluster.capacity < 0.01:
