...
Версия: "Резонансный Перехват" + Пятифазный Взрывной Импульс + Оптимизированный Вывод
...
def main():
    ...
    SHOCKWAVE_START = 1108    # точно на пике дрожания (d=5.210)
    PHASE_RISE1   = 3        # 1108-1110: короткий передний фронт (пока d на пике)
    PHASE_VACUUM  = 4        # 1111-1114: вакуумный пендель (срыв вниз)
    PHASE_RISE2   = 4        # 1115-1118: повторное сжатие
    PHASE_HOLD    = 30       # 1119-1148: заморозка (с запасом)
    PHASE_COOL    = 20       # 1149-1168: остывание
    PULSE_TOTAL   = PHASE_RISE1 + PHASE_VACUUM + PHASE_RISE2 + PHASE_HOLD + PHASE_COOL
    
    start_time = datetime.now()
    print("=" * 70)
    print("3D МОДЕЛИРОВАНИЕ ТАБЛИЦЫ МЕНДЕЛЕЕВА (Пятифазная Версия)")
    ...
    for step in range(start_step, max_steps):
        ...
        # ============================================================
        # ОБЫЧНЫЙ УКОЛ (если shockwave не включён)
        # ============================================================
        ...
        # Вывод: каждый шаг на интервале импульса, каждые 100 — в остальное время
        if args.pulse_shockwave and SHOCKWAVE_START > 0:
            in_pulse_window = (SHOCKWAVE_START - 5 <= step <= SHOCKWAVE_START + PULSE_TOTAL + 20)
        else:
            in_pulse_window = False

        if step % 100 == 0 or step == 0 or in_pulse_window:
        #if True:  # ВЫВОД КАЖДОГО ШАГА (только для этого прогона!)
