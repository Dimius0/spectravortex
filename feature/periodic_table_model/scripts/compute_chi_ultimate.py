...
с оптимизацией всех поправок:
- CHI_0, A (калибровка)
- beta_rel (релятивистская)
- delta_n (нейтронный скин)
- gamma_rad (фрактальное замедление для радиоактивных)
"""
...
# Периоды полураспада (сек) для радиоактивных элементов
HALF_LIVES = {
    84: 1.2e7,      # Po-210: 138 дней
    85: 8.1 * 3600, # At-210: 8.1 часа
    86: 3.3e5,      # Rn-222: 3.8 дня
    87: 1.3e3,      # Fr-223: 22 минуты
    88: 5.0e10,     # Ra-226: 1600 лет
    89: 6.9e8,      # Ac-227: 21.8 лет
    90: 4.4e17,     # Th-232: 14 млрд лет
    91: 1.0e12,     # Pa-231: 32760 лет
    92: 1.4e17,     # U-238: 4.5 млрд лет
    93: 6.6e13,     # Np-237: 2.1 млн лет
    94: 7.6e11,     # Pu-239: 24100 лет
    95: 2.3e10,     # Am-243: 7370 лет
    96: 4.9e15,     # Cm-247: 15.6 млн лет
    97: 4.4e9,      # Bk-247: 1380 лет
    98: 2.8e9,      # Cf-251: 898 лет
    99: 4.1e7,      # Es-252: 471 день
    100: 8.6e6,     # Fm-257: 100 дней
    101: 4.4e6,     # Md-258: 51 день
    102: 3.5e3,     # No-259: 58 минут
    103: 3.9e3      # Lr-266: 11 часов
}
...
# Массовые числа (наиболее распространённые изотопы)
MASS_NUMBERS = {
...
def neutron_skin_correction(Z, delta_n):
    """Поправка на нейтронный скин с оптимизируемым коэффициентом"""
    Z_int = int(Z)
...
def radioactive_slowing(Z, level, gamma_rad):
    """Фрактальное замедление для радиоактивных элементов"""
    Z_int = int(Z)
    if Z_int <= 83:
        return 1.0
    T_half = HALF_LIVES.get(Z_int, 1e10)
    T_0 = 1e10  # характерное время (10 млрд лет ~ возраст Вселенной)
    # Чем меньше период полураспада, тем сильнее замедление
...
def compute_chi_vmms_single(Z, beta_rel, delta_n, gamma_rad):
    ...
    # Базовая частота с фрактальным уровнем
...
def model_function(Z, CHI_0, A, beta_rel, delta_n, gamma_rad):
...
def main():
    ...
    popt, _ = curve_fit(model_function, np.array(Z_exp), np.array(chi_exp), 
                        p0=[1.32, 24.86, 8.09, 0.1, 0.05], maxfev=10000)
    CHI_0_opt, A_opt, beta_opt, delta_opt, gamma_opt = popt
    
    print("=" * 110)
    print("АБСОЛЮТНО ПОЛНАЯ МОДЕЛЬ ЭЛЕКТРООТРИЦАТЕЛЬНОСТИ ВММП")
    print("=" * 110)
    ...
    print(f"delta_n   = {delta_opt:.6f}  (нейтронный скин)")
    print(f"gamma_rad = {gamma_opt:.6f}  (фрактальное замедление радиоактивных)")
