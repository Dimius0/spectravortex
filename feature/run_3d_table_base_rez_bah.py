...
def main():
    ...
    # === ПЕРЕМЕННЫЕ ДЛЯ ОПЕРАЦИИ "УПРЕЖДАЮЩИЙ ШТОРМ" ===
    storm_active = False
    storm_start_step = 24  # Начинаем ДО формирования "распорки"
    storm_T = 10000.0      # Температура "Шторма"
    storm_d_threshold = 2.5 # Порог, после которого считаем "распорку" сломанной
    prev_min_dist = float('inf') # Переменная для хранения min_dist с предыдущего шага
    # =====================================================

    for step in range(start_step, max_steps):
        # === ОПЕРАЦИЯ "УПРЕЖДАЮЩИЙ БИНАУРАЛЬНЫЙ ШТОРМ" (Версия 3.0) ===
        
        # 1. Активация "Шторма" на 24-м шаге
        if step == storm_start_step:
            storm_active = True
            print("=" * 60)
            print(f"[ШТОРМ] ШАГ {step}! АКТИВАЦИЯ УПРЕЖДАЮЩЕГО БИНАУРАЛЬНОГО ЛАЗЕРА!")
            
            # Взламываем температуру
            thermo_state.temperature = storm_T
            if hasattr(thermo_state, 'update_factors'):
                thermo_state.update_factors()
            print(f"[ШТОРМ] Температура поднята до {storm_T} K")

            # Взламываем chi для Fe и Rh НАПРЯМУЮ по символу
            if enable_tuning and chi_tuner:
                # Железо (Fe)
                if 'Fe' in chi_tuner.current_chi:
                    chi_tuner.current_chi['Fe'] = 4.5
                    print("[ШТОРМ] Железо (Fe): chi принудительно установлена в 4.5")
                # Родий (Rh)
                if 'Rh' in chi_tuner.current_chi:
                    chi_tuner.current_chi['Rh'] = 4.5
                    print("[ШТОРМ] Родий (Rh): chi принудительно установлена в 4.5")
            print("=" * 60)

        # 2. Поддержание "Шторма" (Активная Компенсация)
        if storm_active:
            # Проверяем, не сломана ли уже "распорка"
            if prev_min_dist < storm_d_threshold:
                storm_active = False
                print(f"[ШТОРМ] Шаг {step}: d_min = {prev_min_dist:.3f} < {storm_d_threshold}. Распорка сломана! Прекращаем шторм.")
            else:
                # Продолжаем "Шторм": поддерживаем T и chi
                thermo_state.temperature = storm_T
                if hasattr(thermo_state, 'update_factors'):
                    thermo_state.update_factors()
        ...
        # Сохраняем min_dist для проверки на следующем шаге
