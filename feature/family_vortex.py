#!/usr/bin/env python3
"""
family_vortex.py v3.4 — фрактально-эмерджентная модель с бифуркациями.
...
Новое в v3.4:
- AdaptiveClock — адаптивный шаг (Принцип Пульса из Ризомы)
- CheckpointManager — фиксация состояния при взрывах
- AdaptiveFamilyEvolution — расширенный класс с опциями
(по умолчанию адаптация и чекпойнты ВЫКЛЮЧЕНЫ для совместимости)
"""
...
class FractalClock:
    """Фрактальные часы (базовая версия, без адаптации)."""
...
class AdaptiveClock(FractalClock):
    """Фрактальные часы с адаптивным шагом (Принцип Пульса из Ризомы)."""
    
    def __init__(self, n_levels=3, base_dt=0.1, min_dt=0.01, max_dt=0.5):
    ...
    def update_dt(self, global_tension, avg_strain_growth):
        """Адаптация шага по tension и strain_growth."""
        self.tension_history.append(global_tension)
        if len(self.tension_history) > 10:
            self.tension_history.pop(0)
        
        if len(self.tension_history) >= 3:
            trend = self.tension_history[-1] - self.tension_history[-3]
        else:
            trend = 0
        
        if global_tension > 0.5 and trend > 0:
            self.current_dt *= 0.9
        elif global_tension < 0.1 and trend < 0:
            self.current_dt *= 1.05
        else:
            pass
        
        self.current_dt = max(self.min_dt, min(self.max_dt, self.current_dt))
        self.dt_history.append(self.current_dt)
        
        return self.current_dt
    
    def get_dt(self, fractal_level):
        scale = self.level_scales[min(fractal_level - 1, len(self.level_scales) - 1)]
        return self.current_dt * scale


class CheckpointManager:
    """Управление чекпойнтами — фиксация состояния при взрывах."""
    
    def __init__(self, checkpoint_threshold=1000.0):
        self.checkpoint_threshold = checkpoint_threshold
        self.checkpoints = []
        self.last_checkpoint_step = 0
    
    def should_checkpoint(self, evolution, step):
        """Проверяет, нужно ли сделать чекпойнт."""
        max_energy = 0.0
        for c in evolution.clusters:
            if c.energy_history:
                max_energy = max(max_energy, c.energy_history[-1])
        
        if max_energy > self.checkpoint_threshold:
            return True
    ...
    def capture(self, evolution, step):
        """Фиксирует состояние всех кластеров."""
        state = {
            'step': step,
            'clusters': []
        }
        ...
        for c in evolution.clusters:
            state['clusters'].append({
                'name': c.name,
                'tau_charges': c.tau_charges.copy(),
    ...
    def restore(self, evolution, checkpoint_idx=-1):
        """Восстанавливает состояние из чекпойнта."""
        state = self.checkpoints[checkpoint_idx]
        
        for i, c in enumerate(evolution.clusters):
            saved = state['clusters'][i]
            c.tau_charges = saved['tau_charges']
            c.positions = saved['positions']
            c.tension = saved['tension']
            c.strain = saved['strain']
            c.capacity = saved['capacity']
            c.dissolved = saved['dissolved']
            c.born = saved['born']
            c.merged_into = saved['merged_into']
...
class FamilyEvolution:
    """Эволюция семейного кластера v3.4 с бифуркациями (базовая версия)."""
...
class AdaptiveFamilyEvolution(FamilyEvolution):
    """FamilyEvolution с адаптивным шагом и чекпойнтами.
    
    По умолчанию адаптация и чекпойнты ВЫКЛЮЧЕНЫ для совместимости.
    Для включения установите enable_adaptive=True и/или enable_checkpoints=True.
    """
    
    def __init__(self, clusters, box_size=16.0, external_pulses=None,
                 factor_x_list=None, noise_seed=42,
                 enable_adaptive=False, enable_checkpoints=False):
        super().__init__(clusters, box_size, external_pulses, factor_x_list, noise_seed)
        
        self.enable_adaptive = enable_adaptive
        self.enable_checkpoints = enable_checkpoints
        
        if enable_adaptive:
            self.clock = AdaptiveClock()
    ...
    def _compute_global_tension_and_strain(self):
        active = [c for c in self.clusters if c.born and not c.dissolved]
        if not active:
            return 0.0, 0.0
        
        avg_tension = np.mean([c.tension for c in active])
        avg_strain = np.mean([c.strain for c in active]) if active else 0.0
        
        return avg_tension, avg_strain
    
    def evolve_step(self, noise_level=0.005):
        if self.enable_adaptive and isinstance(self.clock, AdaptiveClock):
            global_tension, global_strain = self._compute_global_tension_and_strain()
            self.clock.update_dt(global_tension, global_strain)
        
        if self.enable_checkpoints and self.checkpoint_manager:
            if self.checkpoint_manager.should_checkpoint(self, self.clock.global_step):
                self.checkpoint_manager.capture(self, self.clock.global_step)
                # Можно раскомментировать для отладки:
                # print(f"  [ЧЕКПОЙНТ] зафиксирован на шаге {self.clock.global_step}")
        
        super().evolve_step(noise_level)
