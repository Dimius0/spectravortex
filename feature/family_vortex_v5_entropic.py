#!/usr/bin/env python3
"""
family_vortex_v5_entropic.py — Ризоматическая эволюция с энтропийным стоком.

v5 — добавлен четвёртый механизм теплопереноса:
  - Член стока в уравнении поля: -κ·∇·(ψ·∇ρ)
  - Прямой теплоперенос между кластерами ~1/r
  - Зависимость устойчивости от winding number и окружения
  - Усиление синхронизации при разной "температуре"
...
OMEGA_0 = 2.0 * math.pi
K_POINTS = 20
RESONANCE_THRESHOLD = math.pi / 4
SYNC_DANGER = 0.95
WINDING_JUMP_THRESHOLD = 0.5

# v5: константа энтропийного стока
# Оценка из опыта Гердвиса: ~0.01 для жидкого азота (winding ~ 0)
# Для материалов с кристаллической решёткой (winding > 0) — выше
KAPPA_0 = 0.01  # фундаментальная константа энтропийного стока


def kappa_effective(winding_number: float, material_density: float = 1.0) -> float:
    """Эффективный коэффициент энтропийного стока."""
    return KAPPA_0 * abs(winding_number) * math.sqrt(max(material_density, 0.001))


# ═══════════════════════════════════════════════════════════════
# ВИХРЕВОЙ КЛАСТЕР (без изменений относительно v4)
# ═══════════════════════════════════════════════════════════════
...
class VortexCluster:
    ...
    def __init__(self, birth_time: datetime, birth_position: Tuple[float, float, float],
        ...
        # v5: история энтропийного потока
        self.entropic_flow_history: List[float] = []
...
# ═══════════════════════════════════════════════════════════════
# БИГАРМОНИЧЕСКОЕ ПОЛЕ (v5: с энтропийным стоком)
# ═══════════════════════════════════════════════════════════════

class BiharmonicField:
    """Бигармоническое поле ψ: ∂ψ/∂t = -∇⁴ψ + источники - γψ - κ·∇·(ψ·∇ρ)."""

    def __init__(self, box_size: float = 16.0, grid_size: int = 32, gamma: float = 0.01,
                 kappa_coeff: float = None):
        self.box_size = box_size
        self.grid_size = grid_size
        self.dx = box_size / grid_size
        self.gamma = gamma
        self.kappa_coeff = kappa_coeff if kappa_coeff is not None else KAPPA_0
    ...
    def _compute_entropic_divergence(self) -> np.ndarray:
        """
        v5: Вычисляет дивергенцию энтропийного потока.
        J = ψ·∇ρ, где ρ = |ψ|².
        Возвращает ∇·J в реальном пространстве.
        """
        rho = np.abs(self.psi)**2

        # Градиенты ρ в частотном пространстве
        rho_hat = np.fft.fftn(rho)
        grad_rho_x = np.real(np.fft.ifftn(1j * self.Kx * rho_hat))
        grad_rho_y = np.real(np.fft.ifftn(1j * self.Ky * rho_hat))
        grad_rho_z = np.real(np.fft.ifftn(1j * self.Kz * rho_hat))

        # Поток J = ψ · ∇ρ
        Jx = self.psi * grad_rho_x
        Jy = self.psi * grad_rho_y
        Jz = self.psi * grad_rho_z

        # Дивергенция в частотном пространстве
        Jx_hat = np.fft.fftn(Jx)
        Jy_hat = np.fft.fftn(Jy)
        Jz_hat = np.fft.fftn(Jz)

        div_J_hat = 1j * (self.Kx * Jx_hat + self.Ky * Jy_hat + self.Kz * Jz_hat)
        return np.real(np.fft.ifftn(div_J_hat))
    ...
    def evolve(self, positions_list, charges_list, tensions, dt=0.1,
        ...
        """
        v5: Добавлен член энтропийного стока.
        winding_numbers — список winding number для каждого источника (опционально).
        """
        rho = np.zeros((self.grid_size, self.grid_size, self.grid_size))
        ...
        if winding_numbers is not None:
            avg_winding = np.mean([abs(w) for w in winding_numbers if w is not None]) if winding_numbers else 0.0
        else:
            avg_winding = 0.0
        kappa_eff = kappa_effective(avg_winding, 1.0)

        div_J = self._compute_entropic_divergence()
        div_J_hat = np.fft.fftn(div_J)

        numerator = psi_hat + dt * rho_hat - dt * kappa_eff * div_J_hat
    ...
    def get_gradient_at(self, position: np.ndarray) -> np.ndarray:
        # Проверка на NaN и выход за границы
    ...
    def get_field_value_at(self, position: np.ndarray) -> float:
        ...
        result = ((1 - fx) * (1 - fy) * (1 - fz) * self.psi[i0, j0, k0] +
    ...
    def get_rho_at(self, position: np.ndarray) -> float:
        """v5: Плотность флуктуаций поля H в точке."""
        if np.any(np.isnan(position)):
            return 0.0
        psi_val = self.get_field_value_at(position)
        grad = self.get_gradient_at(position)
        return float(psi_val**2 + np.dot(grad, grad))


# ═══════════════════════════════════════════════════════════════
# ТОПОЛОГИЧЕСКИЙ ИНВАРИАНТ: WINDING NUMBER (без изменений)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# ПУЛЬС: ЭМЕРДЖЕНТНОЕ ВРЕМЯ (МОДЕЛЬ КУРАМОТО) (v5: усиление coupling)
# ═══════════════════════════════════════════════════════════════
...
class PulseClock:
    ...
    def evolve_all(self, field: BiharmonicField, positions: Dict[str, np.ndarray],
                   rho_dict: Dict[str, float] = None):
        """
        Эволюция всех пульсов с coupling через поле.
        v5: добавлен тепловой coupling от разности ρ.
        """
        dt = self.compute_dt()
        ...
        for i, name_i in enumerate(names):
            ...
            for j, name_j in enumerate(names):
                ...
                # v5: тепловая связь — усиление синхронизации при разной "температуре"
                if rho_dict is not None:
                    rho_i = rho_dict.get(name_i, 0.0)
                    rho_j = rho_dict.get(name_j, 0.0)
                    thermal_coupling = KAPPA_0 * abs(rho_i - rho_j) / dist
                    coupling += thermal_coupling * np.sin(dphi)

            pulse_i.evolve(dt, coupling)


# ═══════════════════════════════════════════════════════════════
# ВАРИАЦИОННЫЙ ПРИНЦИП (без изменений)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# ОБМЕННЫЕ СУЩНОСТИ (без изменений)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# ОСНОВНАЯ ЭВОЛЮЦИЯ: РИЗОМА V5 (с энтропийным стоком)
# ═══════════════════════════════════════════════════════════════

class FamilyEvolutionV5:
    """Ризоматическая эволюция v5 — с энтропийным стоком."""

    def __init__(self, clusters: List[VortexCluster],
                 box_size: float = 16.0,
                 external_pulses: List = None,
                 factor_x_list: List = None,
                 enable_entropic_sink: bool = True):
        self.clusters = clusters
        self.box_size = box_size
        self.external_pulses = external_pulses or []
        self.factor_x_list = factor_x_list or []
        self.rng = np.random.RandomState(42)
        self.enable_entropic_sink = enable_entropic_sink  # v5: флаг
    ...
    def _should_dissolve(self, cluster: VortexCluster, velocities: np.ndarray) -> bool:
        ...
        # v5: энтропийная поправка к устойчивости
        if self.enable_entropic_sink:
            rho_env = self.field.get_rho_at(cluster.get_center())
            w = cluster.winding_number or 0.0
            kappa = kappa_effective(w, 1.0)
            entropic_bonus = kappa * abs(w) * rho_env
            E_bind += entropic_bonus  # высокий winding → выше устойчивость в холодной среде
    ...
    def evolve_step(self):
        ...
        # 1. Эволюция поля (v5: с winding numbers)
        dt = self.clock.compute_dt()
        winding_list = [c.winding_number for c in active]
        self.field.evolve(
            [c.positions for c in active],
            [np.array(c.tau_charges) for c in active],
            [c.tension for c in active],
            dt,
            winding_numbers=winding_list
        )

        # 2. Пульс (v5: с rho_dict)
        positions_dict = {c.name: c.get_center() for c in self.clusters if c.born}
        rho_dict = {c.name: self.field.get_rho_at(c.get_center()) for c in self.clusters if c.born}
        self.clock.evolve_all(self.field, positions_dict, rho_dict)
        ...
        for cluster in active:
            ...
            for i in range(cluster.n_vortices):
                new_pos = self._wrap(cluster.positions[i] + velocities[i] * dt)
                if not np.any(np.isnan(new_pos)):
                    cluster.positions[i] = new_pos

            # Энергия
            energies = [self.field.get_energy_density_at(p) for p in cluster.positions[:cluster.n_vortices]]
            energy = float(np.mean(energies))

            # v5: энтропийный поток
            if self.enable_entropic_sink:
                rho_local = float(np.mean([self.field.get_rho_at(p) for p in cluster.positions[:cluster.n_vortices]]))
                rho_env = self.field.get_rho_at(cluster.get_center())
                kappa = kappa_effective(cluster.winding_number or 0.0, 1.0)
                entropic_flow = -kappa * (rho_local - rho_env) * dt
                energy += entropic_flow
                cluster.entropic_flow_history.append(entropic_flow)
            ...
            # Регенерация/деградация зарядов (v5: зависит от направления потока)
            for i in range(cluster.n_vortices):
                ...
                # v5: энтропийная модуляция регенерации
                if self.enable_entropic_sink and cluster.entropic_flow_history:
                    last_flow = cluster.entropic_flow_history[-1]
                    if last_flow < 0:  # теряет энергию
                        regen *= 0.5
                    else:  # поглощает энергию
                        regen *= 1.5
...
# ═══════════════════════════════════════════════════════════════
# ВНЕШНИЕ ВОЗДЕЙСТВИЯ (без изменений)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# ДЕТЕКТОР СОБЫТИЙ (без изменений)
# ═══════════════════════════════════════════════════════════════
