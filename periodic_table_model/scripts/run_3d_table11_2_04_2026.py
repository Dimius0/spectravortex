...
Использование:
    python run_3d_table.py [--steps STEPS] [--grid GRID] [--T TEMP] [--P PRESS] [--resume CHECKPOINT]
"""
...
# ========== ИСПРАВЛЕННАЯ ПОПРАВКА НА ИОНИЗАЦИЮ ==========

k_B_eV = 8.617333262145e-5
OMEGA_0 = 27.2  # эВ, базовая частота (аналог постоянной Ридберга)

def calculate_ionization_alpha(T: float, Z: int) -> float:
    """
    Исправленная функция степени ионизации.
    При T < 1000K возвращает 0.0 (нет ионизации).
    При высоких T рассчитывает вероятность ионизации внешней оболочки.
    """
    if T < 1000:
        return 0.0
    
    # Для высоких температур учитываем ионизацию внешних оболочек
    # Упрощённо: рассматриваем только валентные электроны (n = Z)
    n_valence = min(Z, 8)  # не более 8 валентных электронов
    omega_n = OMEGA_0 * (Z ** 2) / (n_valence ** 2)
    E_bind = omega_n  # энергия связи в эВ
    
    # Вероятность ионизации по Больцману
    P_ion = np.exp(-E_bind / (k_B_eV * T))
    
    # Степень ионизации (0 до 1)
...
def main():
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        # Здесь можно добавить восстановление позиций, если нужно
    ...
    for step in range(start_step, max_steps):
        ...
        print(f"{step+1:5} | {total_energy:12.1f} | {min_dist:6.2f} | {evolved_str:>18} | {alpha_u:6.3f} | {step_time:7.1f}s | {saved:>4}  [{bar}]")
        sys.stdout.flush()

    print("─" * 80)
    print("[4/4] Анализ связей...")
    # Здесь можно добавить финальный анализ связей
