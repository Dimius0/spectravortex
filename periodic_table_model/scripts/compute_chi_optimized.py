...
с оптимизацией калибровочных констант методом наименьших квадратов.
Уточнённая спиновая поправка на основе экспериментальных данных.
"""
...
def get_symmetry(Z):
    Z_int = int(Z)
    if Z_int in SYMMETRIES:
        return SYMMETRIES[Z_int]
    group = ((Z_int - 1) % 18) + 1
...
def zeta(Z):
    Z_int = int(Z)
    return 1.4 * (1 + 10/(Z_int+2))**(-1)

def spin(Z):
    Z_int = int(Z)
    return NUCLEAR_SPINS.get(Z_int, 0.5 if Z_int % 2 == 1 else 0.0)

def compute_chi_vmms_single(Z):
    """Вычисление χ_vmms для одного Z"""
    Z_int = int(Z)
...
def screening_factor(Z):
    Z_int = int(Z)
    r = ATOMIC_RADII.get(Z_int, 2.0)
    z = zeta(Z_int)
    return exp(-r / z)

def model_function(Z, CHI_0, A):
    """Функция для curve_fit - работает с массивами"""
    chi_vmms = np.array([compute_chi_vmms_single(z) for z in Z])
...
def main():
    ...
    for Z in range(1, 104):
        if Z in CHI_PAULING and CHI_PAULING[Z] > 0:
            Z_exp.append(Z)
            chi_exp.append(CHI_PAULING[Z])
    
    Z_exp = np.array(Z_exp)
    chi_exp = np.array(chi_exp)
    
    # Оптимизация
    popt, pcov = curve_fit(model_function, Z_exp, chi_exp, p0=[0.5, 0.3], maxfev=5000)
    CHI_0_opt, A_opt = popt
    
    print("=" * 110)
    print("ОПТИМИЗАЦИЯ КАЛИБРОВОЧНЫХ КОНСТАНТ МЕТОДОМ НАИМЕНЬШИХ КВАДРАТОВ")
    print("=" * 110)
    print(f"CHI_0 (оптимальное) = {CHI_0_opt:.6f}")
    print(f"A (оптимальное)     = {A_opt:.6f}")
    print()
    
    # Расчёт с оптимальными константами
    ...
    for Z in range(1, 104):
        ...
        S = spin(Z)
        chi_vmms = compute_chi_vmms_single(Z)
        delta_omega = (2**(-level)) * symmetry_factor(sym)
        ...
        results.append({
            ...
            'spin': S,
            'level': level,
            'chi_vmms': round(chi_vmms, 4),
            'chi_eff': round(chi_eff, 3),
            'chi_exp': chi_exp
        })
    
    print("=" * 100)
    print("РЕЗУЛЬТАТЫ С ОПТИМИЗИРОВАННЫМИ КОНСТАНТАМИ")
    print("=" * 100)
    print(f"{'Z':>3} | {'Сим':>4} | {'Спин':>4} | {'Ур':>2} | {'χ_vmms':>8} | {'χ_eff':>8} | {'χ_эксп':>8} | {'Δχ':>8}")
    print("-" * 100)
    ...
    for r in results:
        ...
        print(f"{Z:3} | {r['symbol']:>4} | {r['spin']:4.1f} | {r['level']:2} | {r['chi_vmms']:8.4f} | {chi_eff:8.3f} | {exp_str:>8} | {delta_str:>8}")
    ...
    # Сохранение
    output = {
        'calibration': {'CHI_0': CHI_0_opt, 'A': A_opt},
        'results': results,
        'statistics': {'mae': mae, 'R2': corr**2}
    }
    with open('../results/chi_optimized.json', 'w') as f:
        json.dump(output, f, indent=2)
    print("\n✓ Результаты сохранены: ../results/chi_optimized.json")
    print("=" * 100)
