"""
Запуск 3D моделирования таблицы Менделеева с термодинамикой.
ПОЛНАЯ ВЕРСИЯ: 103 элемента, все фрактальные уровни.
Интегрированная версия: T и P влияют на энергию, стабильность и предсказания.
"""
...
def load_all_elements(elements_config):
    ...
    for z in range(1, 104):
        if z not in base_elements:
            ...
            if not found:
                base_elements[z] = base_elements[1]  # Водород как fallback
    ...
    for Z in range(32, 104):
        ...
        # Масштабируем частоту и электроотрицательность с периодом
...
def compute_vortex_interaction_energy(comp1, comp2, distance, state: ThermodynamicState):
    """
    Вычислить энергию взаимодействия двух вихрей с учётом T и P.
    """
    # Базовое кулоновское отталкивание
    q1, q2 = comp1['charge'], comp2['charge']
    E_coulomb = q1 * q2 / (distance + 1e-6)
    
    # Вихревое притяжение (зависит от симметрии)
...
def main():
    ...
    print("=" * 70)
    print("3D МОДЕЛИРОВАНИЕ ТАБЛИЦЫ МЕНДЕЛЕЕВА С ТЕРМОДИНАМИКОЙ")
    print("Решатель: BiharmonicSolver3D + Thermodynamics")
    print("=" * 70)
    ...
    # Создание термодинамического состояния
    ...
    print(f"  - Группы симметрии: {dict(sorted(sym_groups.items(), key=lambda x: -x[1])[:5])}")
    ...
    # ========== ОПТИМИЗАЦИЯ ==========
    print(f"\n[3/4] Оптимизация ({max_iter} итераций)...")
    print("=" * 70)
    print(f"{'Итер':>4} | {'Энергия':>15} | {'ΔE':>12} | {'Мин.расст':>10} | {'Время':>8}")
    print("-" * 70)
    ...
    for iteration in range(max_iter):
        ...
        if iteration == 0:
            architect.solve_biharmonic(max_iter=30, tol=1e-4)
            energy = architect.compute_energy()
        else:
            architect.relax_vortices(max_iter=1, learning_rate=0.03, temperature=0.1)
            architect.solve_biharmonic(max_iter=30, tol=1e-4)
            energy = architect.compute_energy()
        
        energy_history.append(energy)
        
        delta_e = energy - prev_energy if prev_energy is not None else 0
        delta_str = f"{delta_e:+.2e}" if prev_energy is not None else "-"
        
        positions = [c['vortex'].position for c in architect.components]
        min_dist = min(np.linalg.norm(positions[i] - positions[j])
                      for i in range(len(positions))
                      for j in range(i+1, len(positions))) if len(positions) > 1 else float('inf')
        
        iter_time = (datetime.now() - iter_start).total_seconds()
        prev_energy = energy
        
        print(f"{iteration:4} | {energy:15.2f} | {delta_str:>12} | {min_dist:10.2f} | {iter_time:7.2f}s")
        
        if save_checkpoints and (iteration + 1) % checkpoint_interval == 0:
            checkpoint_file = os.path.join(results_dir, f'checkpoint_thermo_{iteration+1}.json')
    ...
    # ========== АНАЛИЗ СВЯЗЕЙ С ТЕРМОДИНАМИКОЙ ==========
    print("\n[4/4] Анализ связей с термодинамикой...")
    
    architect.solve_biharmonic(max_iter=50, tol=1e-6)
    final_energy = architect.compute_energy()
    ...
    for i in range(len(positions)):
        for j in range(i+1, len(positions)):
            ...
            if dist < 15.0:
                comp1 = components_data[i]
                comp2 = components_data[j]
                
                # Энергия взаимодействия с учётом T и P
                E_interaction = compute_vortex_interaction_energy(
                    comp1, comp2, dist, thermo_state
                )
                
                # Совместимость и резонанс
                sym_compat = symmetry_compatibility(
                    comp1['data'].get('symmetry_group', 'C∞v'),
                    comp2['data'].get('symmetry_group', 'C∞v')
                )
                freq_res = frequency_resonance(comp1, comp2)
    ...
    results = {
        'metadata': {
            'model': 'VMMS 3D + Thermodynamics',
            'grid_size': grid_size,
            'iterations': max_iter,
            'num_elements': len(all_elements),
            'T_K': T,
            'P_GPa': P,
            'f_T': thermo_state.temperature_factor,
            'f_P': thermo_state.pressure_factor,
        ...
        'thermodynamics': {
            'thermal_energy_eV': thermo_state.thermal_energy,
            'T_lambda': thermo_state.T_lambda,
            'K0_GPa': thermo_state.K0
        },
    ...
    output_file = os.path.join(results_dir, f'results_thermo_T{T}_P{P}_{grid_size}_full.json')
    ...
    print(f"  - Финальная: {final_energy:.2f}")
    
    if energy_history[0] != 0:
        reduction = (energy_history[0] - final_energy) / energy_history[0] * 100
        print(f"  - Снижение: {energy_history[0] - final_energy:.2f} ({reduction:.2f}%)")
    ...
    if bond_details:
        ...
        for b in bond_details[:10]:
            print(f"  {b['elements'][0]:>2}-{b['elements'][1]:<2} (Z={b['Z'][0]:2}-{b['Z'][1]:<2}): "
