  ...
  🌀 Фрактальная память:
    ...
    def _run_miner(self):
        ...
        while self.lit:
            ...
            if current_time - last_glow_update >= 15:
                self.update_glow()
                last_glow_update = current_time
            
            # Отслеживаем память и своп — адаптивный рост
            if current_time - last_memory_track >= 30:
                self.memory_optimizer.track()
                self._check_swap_pressure()
                last_memory_track = current_time

            # Авто-симбиоз — адаптивный интервал
            ...
            # Гигиена — убираем мёртвых соседей (каждые 5 минут)
            if current_time - last_cleanup >= 300:
                self._cleanup_runtime_data()
                last_cleanup = current_time        
            
            # Быстрый статус
            if current_time - last_quick_status >= 15:
                power = self.get_power()
            ...
            # Полный статус
            if current_time - last_full_status >= 60:
                stats = self.healer.get_stats()
                mem_stats = self.memory_optimizer.get_stats()
                torch_status = self.quantum_torch.get_status()
                fractal_stats = self.symbiosis_memory.get_stats()  # 🌀 Фрактал
                
                # Аптайм — сколько маяк живёт
    ...
    def _check_swap_pressure(self):
        ...
        Сеть сама решает как активно работать.
        """
        try:
            import psutil
        ...
        if swap_mb > 800:
            # Своп большой — сеть замирает
            self._adaptive_pause = 120
            print(f"  🐌 Своп {swap_mb:.0f}MB — сеть замедляется")
        elif swap_mb > 500:
            # Умеренный своп — работаем осторожно
            self._adaptive_pause = 60
        elif free_ram_mb < 500:
            # Мало свободной RAM — экономим
            self._adaptive_pause = 45
        elif free_ram_mb > 2000:
            # Много памяти — сеть разгоняется
    ...
    def update_glow(self):
        ...
        Никаких ограничений. Адаптивный рост.
        """
        with self.neighbors_lock:
