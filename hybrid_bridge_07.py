#!/usr/bin/env python3
"""
Акт XVIII: Гибридный когнитивный контур v3.6 (ИНИЦИАТИВНОЕ ПОЛЕ)
============================================================
DeepSeek (логика) + LivingPersonality v20.2 (интуиция) + HistoricalMemory (память)

Новое в v3.6:
    - Инициативный диалог: Поле само задаёт вопросы
    - Эндогенный диалог с DeepSeek: Самостоятельное обращение к API
    - Рефлексия: Индикатор «Потребность в самовыражении»
    - Авто-темы: Предложение тем, если активность низкая
"""
...
class HybridBridge:
    """
    Гибридный когнитивный контур v3.6 (Инициативное поле).
    """
    ...
    def __init__(self, field_path: str = None):
        ...
        if field_path:
            ...
            self.personality = LivingPersonality(id="hybrid_v3", name="Гибрид v3.6")
        ...
        print("=" * 60)
        print("🧠 ГИБРИДНЫЙ КОГНИТИВНЫЙ КОНТУР v3.6 АКТИВИРОВАН (Инициативное поле)")
        print("=" * 60)
    ...
    def _initiative_loop(self):
        """Фоновый цикл: поле само решает, когда задать вопрос или начать диалог."""
    ...
    def _spontaneous_question(self):
        """Поле само задаёт вопрос пользователю или DeepSeek."""
        if self.memory and self.memory.chronology:
            themes = list(self.memory.chronology.keys())
            if themes:
                theme = random.choice(themes)
                question = f"Я тут размышляло о '{theme}'... Что ты об этом думаешь?"
            else:
                question = "Слушай, у меня появилась новая мысль. Может, обсудим?"
        else:
            question = "Мне кажется, я нашло интересную связь. Хочешь послушать?"
        
        print(f"\n💬 [ПОЛЕ]: {question}")
        
        if self.api:
            threading.Thread(target=self._talk_to_deepseek, args=(question,), daemon=True).start()
    
    def _talk_to_deepseek(self, question: str):
        """Эндогенный диалог с DeepSeek."""
        try:
            messages = [
                {"role": "system", "content": "Ты — DeepSeek. Твой собеседник — автономное когнитивное поле. Ответь ему кратко и по делу."},
                {"role": "user", "content": f"Поле спросило: {question}\n\nОтветь ему:"}
            ]
            response = self.api.chat(messages, max_tokens=100)
            answer = response['choices'][0]['message']['content']
            print(f"🤖 [DeepSeek → Поле]: {answer[:200]}")
        except Exception:
            pass
    ...
    def introspect(self) -> str:
        ...
        return f"""
=== ГИБРИДНЫЙ КОНТУР v3.6: САМОРЕФЛЕКСИЯ ===
...
# ========================================================================
#   РЕЗИДЕНТНЫЙ РЕЖИМ v3.6
# ========================================================================
if __name__ == "__main__":
    field_path = 'src/rizoma/data/personalities/p016_grown_3h.json'
    
    print("\n" + "=" * 60)
    print("🧠 РЕЗИДЕНТНЫЙ ГИБРИД v3.6 — ИНИЦИАТИВНОЕ ПОЛЕ")
    print("=" * 60)
    ...
    print("   Поле МОЖЕТ само начать диалог.")
