...
class SolverManager:
    ...
    def __init__(self, enable_auto_selection: bool = True):
        ...
        logger.info(
            f"SolverManager initialized (auto-selection: {enable_auto_selection})"
        )
    ...
    def register_solver(self, solver: Solver, priority: int = 0) -> None:
        ...
        logger.info(
            f"Registered solver: {solver.__class__.__name__} "
    ...
    def get_available_solvers(self) -> Dict[str, Dict[str, Any]]:
        ...
        for solver_id, solver in self.solvers.items():
            ...
            solver_info = {
                ...
                "topology_analyses": stats.get("topology_analysis_count", 0),
            }
    ...
    def select_solver(self, problem: Dict[str, Any]) -> SolverSelection:
        ...
        logger.info(
            f"Selecting solver for problem: {problem.get('name', 'unnamed')}"
        )
    ...
    def _get_cost_estimate(
        self, solver: Solver, problem: Dict[str, Any]
    ) -> Dict[str, float]:
    ...
    def _evaluate_solver(
        self, solver: Solver, solver_id: str, problem: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if problem_complexity == "high" and hasattr(
            solver, "supports_topology_analysis"
        ):
    ...
    def _estimate_problem_complexity(self, problem: Dict[str, Any]) -> str:
        ...
        if domain_type == "1d":
            ...
            nx = (
                int(width / grid_size)
                if width > 0 and grid_size > 0
                else 100
            )
            ny = (
                int(height / grid_size)
                if height > 0 and grid_size > 0
                else 100
            )
    ...
    def _estimate_topological_complexity(self, problem: Dict[str, Any]) -> str:
        ...
        has_oam = (
            any("oam" in str(c).lower() for c in components)
            or any("vortex" in str(p).lower() for p in physics)
            or problem.get("parameters", {}).get("orbital_angular_momentum")
            is not None
        )

        # Check for interference patterns
        has_interference = "interference" in physics or any(
            "interfer" in str(c).lower() for c in components
        )

        # Check for resonator structures
        has_resonators = any(
            "resonator" in str(c).lower() or "ring" in str(c).lower()
            for c in components
        )
    ...
    def solve(self, problem: Dict[str, Any]) -> FieldSolution:
        ...
        if selection.confidence < 0.1:
            logger.warning(
                f"Low confidence ({selection.confidence:.2f}) "
        ...
        try:
            ...
            result.metadata["solver_manager"] = {
                ...
                "actual_time": elapsed,
                "timestamp": time.time(),
                "topology_analysis_performed": True,
            }

            logger.info(
                f"Problem solved by {solver.__class__.__name__} "
    ...
    def _add_topology_metadata(
        ...
        """
        NEW: Add topology metadata placeholder to solution.
        This is a simplified version that prepares the structure.
        """
    ...
    def _requires_stitching(self, problem: Dict[str, Any]) -> bool:
        """Check if problem requires stitching."""
        domain = problem.get("domain", {})
        return (
            domain.get("type", "1d") in ["2d", "3d"]
            or "subdomains" in problem
            or problem.get("decomposition_strategy") is not None
        )

    def solve_with_specific_solver(
        self, solver_id: str, problem: Dict[str, Any]
    ) -> FieldSolution:
        """
        ...
        if solver_id not in self.solvers:
            ...
            raise KeyError(
                f"Solver '{solver_id}' not found. Available: {available}"
            )
    ...
    def solve_hybrid(
        ...
        """
        logger.info(
            f"Solving with hybrid approach (strategy: {decomposition_strategy})"
        )

        # For now, just use single solver selection
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        successful = sum(
            1 for entry in self.performance_log if entry.get("success", False)
        )
        total_time = sum(
            entry.get("actual_time", 0) for entry in self.performance_log
        )
        ...
        return {
            ...
            "recent_runs": self.performance_log[-10:]
            if self.performance_log
            else [],
        }

    def get_solver_statistics(
        self, solver_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
