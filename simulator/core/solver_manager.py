...
class SolverManager:
    ...
    def register_solver(self, solver: Solver, priority: int = 0) -> None:
        ...
        solver_id = f"{solver.name}_{id(solver)}"
        ...
        logger.info(f"Registered solver: {solver.name} v{solver.version} (ID: {solver_id})")
    ...
    def get_available_solvers(self) -> Dict[str, Dict[str, Any]]:
        ...
        for solver_id, solver in self.solvers.items():
            ...
            result[solver_id] = {
                ...
                'max_dimensions': requirements.get('max_dimensions', 0),
    ...
    def select_solver(self, problem: Dict[str, Any]) -> SolverSelection:
        ...
        if not self.enable_auto_selection:
            ...
            return SolverSelection(
                ...
                estimated_cost=first_solver.estimate_computation_cost(problem),
            )
        ...
        if len(evaluations) > 1:
            ...
            if best_eval['score'] > 0:
                confidence = min(1.0, (best_eval['score'] - second_best) / best_eval['score'])
    ...
    def _evaluate_solver(self, solver: Solver, solver_id: str, problem: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not can_solve:
            return {
                ...
                'estimated_cost': {'memory_mb': 0, 'time_seconds': 0, 'complexity': 0},
            }
        ...
        # Adjust based on problem complexity match
        problem_complexity = self._estimate_problem_complexity(problem)
        solver_capabilities = solver.get_requirements()
        solver_max_dims = solver_capabilities.get('max_dimensions', 0)
        
        # Penalize overqualified solvers for simple problems
        if problem_complexity == 'simple' and solver_max_dims > 2:
            score *= 0.8
        
        # Get cost estimate
        cost_estimate = solver.estimate_computation_cost(problem)
        ...
        return {
            ...
            'reason': f"Compatible: {reason}. Priority: {priority}, Success: {success_rate:.1%}",
    ...
    def solve(self, problem: Dict[str, Any]) -> FieldSolution:
        ...
        solver_id = self._get_solver_id(solver)
        
        if selection.confidence < 0.1:
            logger.warning(f"Low confidence ({selection.confidence:.2f}) for solver selection: {selection.reason}")
        ...
        try:
            ...
            logger.info(f"Problem solved by {solver.name} in {elapsed:.3f}s "
            ...
            logger.error(f"Solver {solver.name} failed: {e}")
            raise
    ...
    def solve_with_specific_solver(self, 
        ...
        can_solve, reason = solver.can_solve(problem)
        if not can_solve:
            raise ValueError(f"Solver {solver.name} cannot solve this problem: {reason}")
    ...
    def decompose_problem(self, 
        ...
        if decomposition_strategy == 'auto':
            ...
            if domain_type == '2d':
                ...
                subdomains = [
                    ('bottom_left', {'x_min': 0, 'x_max': width/2, 'y_min': 0, 'y_max': height/2}),
                    ('bottom_right', {'x_min': width/2, 'x_max': width, 'y_min': 0, 'y_max': height/2}),
                    ('top_left', {'x_min': 0, 'x_max': width/2, 'y_min': height/2, 'y_max': height}),
                    ('top_right', {'x_min': width/2, 'x_max': width, 'y_min': height/2, 'y_max': height}),
                ]
    ...
    def _get_solver_id(self, solver: Solver) -> str:
        """Get ID for a solver instance."""
        for solver_id, s in self.solvers.items():
            if s is solver:
                return solver_id
        return f"{solver.name}_{id(solver)}"
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if not self.performance_log:
            return {'total_runs': 0, 'average_time': 0.0, 'success_rate': 0.0}
        
        total_runs = len(self.performance_log)
        successful = sum(1 for entry in self.performance_log if entry.get('success', False))
        total_time = sum(entry.get('actual_time', 0) for entry in self.performance_log)
    ...
    def get_solver_statistics(self, solver_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            'summary': {
                ...
                for solver_id, stats in self.solver_stats.items()
            }
        }
