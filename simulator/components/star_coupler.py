...
class StarCoupler(OpticalComponent):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized StarCoupler '{name}' with "
            f"OAM l={self.params['topological_charge']}"
        )
    ...
    def _calculate_geometry(self):
        ...
        topological_charge = self.params["topological_charge"]  # Исправлено: было 'l'
        ...
        self.port_phases = topological_charge * self.port_angles  # Исправлено

        # Cartesian coordinates for output ports
        self.port_positions = radius * np.array(
            [np.cos(self.port_angles), np.sin(self.port_angles)]
        ).T

        logger.debug(
            f"StarCoupler geometry: {n_ports} ports at radius {radius*1e6:.1f} um"
        )

    def propagate_field(
        self, input_field: np.ndarray, grid_info: Dict[str, Any]
    ) -> np.ndarray:
        """
        ...
        if len(x) > 1 and len(y) > 1:
            ...
            topological_charge = self.params["topological_charge"]  # Исправлено: было 'l'
            phase = topological_charge * Theta  # Исправлено
    ...
    def get_s_matrix(self, frequency: float) -> np.ndarray:
        ...
        """
        n_ports = self.params["ports"]
        # Убрана неиспользуемая переменная 'l' (была в строке 125)
        ...
        for k in range(n_ports):
            ...
            phase = self.port_phases[k]  # Используем уже рассчитанные фазы
