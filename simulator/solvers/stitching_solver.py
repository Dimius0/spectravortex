...
from ..core.solver_manager import TopologicalFeature
...
class StitchingSolver(Solver):
    ...
    def can_solve(self, problem: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        Args:
            problem: Must contain 'subdomain_solutions' key with list of solutions
            
        Returns:
            (can_solve, reason)
        """
        if "subdomain_solutions" not in problem:
            return False, "No subdomain solutions provided"
        
        solutions = problem.get("subdomain_solutions", [])
        if len(solutions) < 2:
            return False, "Need at least 2 solutions to stitch"
        
        # Check that all solutions have compatible fields
        first_sol = solutions[0]
        for i, sol in enumerate(solutions[1:], 1):
            if not hasattr(sol, 'amplitude') or not hasattr(sol, 'phase'):
                return False, f"Solution {i} missing amplitude or phase fields"
            
            # Check dimensionality compatibility
            if sol.amplitude.ndim != first_sol.amplitude.ndim:
                return False, f"Solution {i} has different dimensionality"
        
        return True, f"Can stitch {len(solutions)} subdomain solutions"
                ...
                'features': [
                    {
                        ...
                        'position': f.position,
                        'strength': f.strength,
                        'stability': f.stability_index,
                        **f.metadata,
                    }
                    for f in topology_features
                ],
                'total_features': len(topology_features),
