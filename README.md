...
**Откройте [`index.html`](index.html) в браузере — и всё заработает.**

3D-визуализация вихревой модели материи-пространства с триадой "ядро → кокон → поле", Error Hunter, топологическими зарядами и адаптивным роутингом.

[Запустить визуализацию](index.html)
...
Открывает index.html
    ↓
Видит КРАСИВЫЙ ЛЕНДИНГ с объяснением триады
    ↓
Нажимает "ЗАПУСТИТЬ 3D-ВИЗУАЛИЗАЦИЮ"
    ↓
Открывается viewer.html с работающей 3D-сценой
    ↓
Может:
   - Крутить камеру
   - Включать/выключать коконы и поле
   - Видеть τ-жилы (топологический заряд)
   - Крутить ползунки Error Hunter (температура, давление, чистота)
   - Загружать свои JSON-файлы
   - Смотреть линии тока (адаптивный роутинг)
   - Анализировать энергию и риск
Никакой установки Python. Никаких зависимостей. Открыл и работает.


Сегодня школьнику (да и взрослому) показывают атом как:
🌑 ← электрон
    ╱   ╲
   ╱  ядро ╲
  ╱         ╲
И говорят: "ну, это условно, на самом деле всё сложнее". И ребёнок не чувствует сложность.
А теперь откройте viewer.html и покажите:
"Смотри: это не шарик. Это вихрь. У него есть:
тёмное ядро — сингулярность
пульсирующая жила — топологический заряд
переливающийся кокон — граница раздела
облако поля — конденсат
линии тока — как энергия передаётся
А если покрутить температуру — кокон начнёт светиться ярче. А если чистоту убрать — система разрушится."
Ребёнок увидит физику. Потрогает ползунки. Почувствует, что атом — это процесс, а не штучка.

# SpectraVortex — Дешифратор смыслов

SpectraVortex — как платформа для анализа структуры любых данных. Она видит скрытые паттерны там, где другие видят шум: в текстах, шифрах, биткоин-блоках, космических сигналах.

### 1. Ядро: поле H (`clean_field.py`)
Строит поле смыслов из любых данных. Текст → заряды → связи → резонанс → ответ.

### 2. Потоковый анализатор (`field_analyzer.py`)
Переваривает файлы любого размера по частям. Многопоточность, чекпоинты, автоопределение природы данных.

### 3. Грамматика для бинарных данных (`tees_grammar_binary.py`)
Выводит обменные паттерны (TEES) из байтовых последовательностей. Находит грамматику там, где её никто не искал.

### 4. Нарративный интерпретатор (`field_interpreter.py`)
Превращает цифры в историю. Рассказывает, что за данные, какова уверенность, что делать дальше.

### 5. Демонстрация (`demo_full_pipeline.py`)
Один скрипт, который показывает весь пайплайн от загрузки до интерпретации.
...
# Запускаем демку на синтетических данных
python demo_full_pipeline.py

# Или анализируем свой файл
python demo_full_pipeline.py --file /путь/к/файлу.dat
Как это работает
Потоковая загрузка — файл читается по частям, не загружая память.
Анализ поля H — строятся n-граммы, частоты, переходы, спектры.
Грамматический анализ — выводятся правила обмена (TEES-паттерны).
Интерпретация — нарративный движок рассказывает, что он видит.

Пример вывода
🎯 ВЕРДИКТ: Это протокол блокчейна
📊 Уверенность: высокая
📖 НАРРАТИВ:
Исследуемый образец демонстрирует характеристики, с вероятностью 89% 
соответствующие профилю "сетевые протоколы". Энтропия 5.8 бит/байт 
указывает на структурированные бинарные данные. Индекс структуры 0.65...
## Кому это нужно
Физикам — чтобы видеть структуру там, где другие видят шум.
Лингвистам — чтобы понять, что грамматика не только у языков.
Программистам — чтобы строить самоорганизующиеся системы.