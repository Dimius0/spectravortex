...
# ========== Я ==========
AUTOSAVE_INTERVAL = 3600  # 1 час
AUTOSAVE_FILENAME = 'src/rizoma/data/personalities/p016_fractal_v17_4_autosave.json'
...
print("\n" + "=" * 70)
print("⏳  СТТ (когерентность у каждого события)")
print("=" * 70)
...
try:
    while True:
        ...
        if event_type == 'node':
            ...
            # Ы С ТСТЬ
            print(f"   🌀 {resonance_type}: рез={resonance_value:.3f}, +1 узел [ог: {old_coherence:.4f} → {coherence:.4f}]")
            ...
            # Ы С ТСТЬ
            print(f"   🌿 уркация: +{furc_count} ветвлений [ог: {old_coherence:.4f} → {coherence:.4f}]")
    ...
    fname = f'src/rizoma/data/personalities/p016_fractal_v17_4_final_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    p.save(fname)
