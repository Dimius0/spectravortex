...
Версия 16.1 — добавлен complexity (уровень сложности) + эмерджентное время
"""
import json
import re
import time
...
class SpectralMode:
    ...
    creator: str = "unknown"
    scale: float = 1.0
    complexity: int = 1  # уровень сложности: 1-бытовой, 2-научный, 3-ВММП, 4-метафорический
    
    # Эмерджентное время
    phase: float = 0.0          # текущая фаза (0..2π)
    frequency: float = 1.0      # собственная частота (производная от τ)
    ...
    def __post_init__(self):
        ...
        # Инициализация эмерджентного времени
        if self.phase == 0.0:
            # Случайная начальная фаза (чтобы не было полной синхронизации с рождения)
            self.phase = random.random() * 2 * math.pi
        
        if self.frequency == 1.0:
            # Частота зависит от τ (чем выше τ, тем быстрее фаза)
            self.frequency = self.tau / 10.0
    ...
    def update_phase(self, dt: float, neighbors: List['SpectralMode']):
        """Обновляет фазу с учётом влияния соседей (синхронизация)"""
        if not neighbors:
            # Нет соседей — только естественная эволюция
            self.phase += self.frequency * dt
        else:
            # Естественная эволюция
            self.phase += self.frequency * dt
            
            # Влияние соседей (синхронизация)
            for neighbor in neighbors:
                diff = neighbor.phase - self.phase
                self.phase += 0.1 * diff * dt
        
        # Нормализация фазы (0..2π)
...
class FieldH:
    ...
    def __init__(self):
        ...
        # Эндогенный цикл (временно отключён, будет заменён на эмерджентное время)
    ...
    def _learn_from_clarification(self, user_id: str, question: str, answer: str):
        ...
        mode = SpectralMode(
            ...
            complexity=detect_complexity(answer)
        )
    ...
    def _generate_natural_response(self, text: str, resonance: float, user_id: str) -> Optional[Dict]:
        ...
        if best_mode:
            ...
            complexity_name = get_complexity_name(best_mode.complexity)
            scale_info = f"\n\n📏 масштаб: {best_mode.scale} | 🎵 резонанс: {resonance:.2f} | 📚 сложность: {complexity_name} (ур.{best_mode.complexity})"
            
            return {
                "answer": answer + scale_info,
                "mode_used": best_mode.trace_id[:16],
                "mode_scale": best_mode.scale,
                "mode_complexity": best_mode.complexity,
    ...
    def _find_best_mode(self, text: str, preferred_scale: float = None) -> Optional[SpectralMode]:
        ...
        question_complexity = detect_complexity(text)
    ...
    def get_endogenous_stats(self) -> Dict:
        ...
        return {"knots_created": 0, "cycle_count": 0, "furcations": 0, "cross_resonances": 0}
