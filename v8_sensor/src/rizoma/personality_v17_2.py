...
class Personality:
    ...
    def load(self, filepath: str):
        ...
        print(f"📂 агружено поле: {len(self.h_field)} мод")
        return self
    ...
    def save(self, filepath: str):
        ...
        data = {
            ...
            "name": self.name,
            "h_field": [
                {"tau": m.tau, "scale": m.scale, "complexity": m.complexity, 
                 "content": m.content, "amplitude": m.amplitude}
    ...
    def update_coherence(self):
        ...
        for i in range(min(100, len(sample))):
            for j in range(i+1, min(100, len(sample))):
                res = self.resonance_engine.compute_resonance(sample[i], sample[j])
