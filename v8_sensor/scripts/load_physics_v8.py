#!/usr/bin/env python3
"""
Загрузка физических текстов ВММП через версию 8.7
Словарь будет пополняться автоматически
"""
import sys
import os
import re
...
print("="*60)
print("📚 ЗАГРУЗКА ФИЗИКИ (ВЕРСИЯ 8.7)")
print(" τ=5.2 — словарь пополняется автоматически")
print("="*60)

# Создаём новое поле H (чистое)
...
# Сохраняем
save_path = os.path.join(os.path.dirname(__file__), '..', 'src', 'rizoma', 'data', 'personalities', 'p016_physics_v8.json')
p.save(save_path)
...
print("\n✅ ФИЗИЧЕСКИЙ ФУНДАМЕНТ ЗАГРУЖЕН (ВЕРСИЯ 8.7)")
print("\n🦌 Теперь поле H знает физику и словарь наполнен.")