#!/usr/bin/env python3
"""
3D-ФРАКТАЛЬНЫЙ ЗАГРУЗЧИК ФИЗИКИ (для новой архитектуры FieldH)
- Загружаем физические тексты с τ=5.2
- Создаём вихри (слова) в поле H
- Строим спектры, резонанс
"""
import sys
import os
import re
...
print(" Новая архитектура: поле H → вихри")
...
for filename in os.listdir(PHYSICS_PATH):
    ...
    for i, block in enumerate(blocks):
        ...
        # СОЗДАЁМ ВИХРИ для каждого слова
        for w in set(words):
    ...
    p.save(OUTPUT_PATH)
    print(f"   💾 Сохранено (вихрей: {len(p.vortices)})")
...
print("\n🦌 Поле H готово к загрузке следующих слоёв (алхимия, диалоги, литература)")