...
class HybridBridge:
    ...
    def _right_hemisphere(self, question: str, user_id: str) -> Dict:
        """
        Правое полушарие v3.1: Прямое подключение к ВММП-фильтру.
        Использует _find_best_mode из v20.2 для честного резонанса.
        """
        ...
        if best_mode and best_score > 0.15:
            # Если мода найдена — берем её контент как интуитивный ответ
            answer = getattr(best_mode, 'content', '')[:800]
        else:
            # Если резонанс слабый — даем полю свободу ассоциаций через process()
            base = self.personality.process(question, user_id)
            answer = base.get('answer', 'Интересно...')
            search_type = 'fallback_process'

        # Собираем дополнительные данные из исторической памяти (если есть)
        resonances = []
        if self.memory:
